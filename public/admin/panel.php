...
<div class="ui main container" style="margin-top: 64px">
    ...
    <?php }
        ...
        <div class="ui form">
            <div class="field">
                <textarea id="json" rows="30"></textarea>
            </div>
            <div class="field">
                <div class="ui primary button" onclick="update_json()" id="submit"><i class="chevron circle up icon"></i>提交修改</div>
                <div class="ui button" onclick="hide_msg(); get_json()" id="refresh"><i class="sync alternate icon"></i>刷新</div>
                <div class="ui red button" onclick="set_to_def()" id="s2def"><i class="undo icon"></i>重置</div>
            </div>
        </div>
        <script>
            let jsontxt = $("#json");
            ...
            function get_json() {
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        console.log(data);
                        jsontxt.val(JSON.stringify(JSON.parse(data), null, 4));
