...
if (!empty($_GET['logout']) && $_GET['logout'] == 1) {
    ...
    $logout = empty($logout) ? "http://".Info::$domain : $logout;
    redirect($logout);
    die;
}
...
switch ($type) {
    case "register": {
        ...
        if (!is_tinytext($email)) die("邮箱长度限制为0~255字符，你的邮箱长度为".strlen($email));
        if (!preg_match("/[\w\-_]+@[\w\-_]+.\w+/", $email)) die("邮箱格式错误");
        if (!check_length($password, 6, 100)) die("请填写密码（长度6~100）");
        if (!is_tinytext($nickname)) die("昵称长度限制为0~255字符，你的昵称长度为".strlen($nickname));
        if (!preg_match("/[\w\-_]+/", $email)) die("昵称仅限a-zA-z0-9以及减号和下划线");
    ...
    case "login": {
        ...
        if (!is_tinytext($email)) die("邮箱长度限制为0~255字符，你输入的邮箱长度为".strlen($email));
    ...
    case "manage": {
        ...
        $nickname = $_POST['nickname'];
        if (!is_tinytext($nickname)) die("昵称长度限制为0~255字符，你的昵称长度为".strlen($nickname));
        $intro = empty($_POST['intro']) ? $config['def_user_desc'] : $_POST['intro'];
