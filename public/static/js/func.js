...
function redirect_to_from() {
    ...
    if (url.length > 0) {
        window.location.replace(get_from_url());
    }
}
function get_from_url() {
    ...
    to = to == null ? "http://" + window.location.host : cur_url_decode(to[0].split("=")[1]);
    return to;
}
...
function user_card_obj_to_str(obj) {
    return '\
    <div class="card">\
        <a class="image" href="/user/space.php?uid=' + obj['uid'] + '">\
          <img src="https://cravatar.cn/avatar/' + obj['emmd5'] + '?d=<?php echo Info::$def_user_avatar ?>" alt="avatar">\
        </a>\
